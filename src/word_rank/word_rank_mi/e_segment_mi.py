...
class DNASegment:
    ...
    def get_unkonw_word_prob(self, word):
        return math.log(1./(self.all_freq*10**len(word)))
    ...
    def mp_seg(self, sequence):
        ...
        seg_prob = node_state_list[-1]["prob_sum"] #分词的概率
        seg_sequence = DELIMITER.join(word_list)
        return (seg_prob, seg_sequence)
    ...
    def initial_dict(self,filename):
        ...
        self.gmax_word_length = max(len(key) for key in self.word_dict_count.iterkeys()) #所有词的词频
        #计算每个词的概率，用log形式
    ...
    #计算词的mi
    def get_word_mi(self, word):
        if self.word_dict.has_key(word):
            word_prob = self.word_dict[word]
            self.word_dict[word] =  self.get_unkonw_word_prob(word) #给一个很小的概率
            (seg_prob, seg) = self.mp_seg(word)
            mi = word_prob - seg_prob
            self.word_dict[word] = word_prob #恢复原来的值
        else:
            mi = -200
        return mi
#test
if __name__=='__main__':
    myseg = DNASegment()
    myseg.initial_dict("dict_0.sort.300wan")
    sequence = "AA"
    word_mi = myseg.get_word_mi(sequence)
    print "word mi: " , word_mi
