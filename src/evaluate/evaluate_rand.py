...
#将分割节点随机化
def get_rand_seg(seg_pos_list):
    rand_seg_list = []
    rand_seg_list.append(seg_pos_list[0]) #first node,=0
    max_node_index = seg_pos_list[-1] - 1 #节点的最大index
    for i in range(len(seg_pos_list) -2 ):
        rand_pos = random.randint(1, max_node_index)
        rand_seg_list.append(rand_pos)

    rand_seg_list.append(seg_pos_list[-1])#last node
    return rand_seg_list
...
def get_seg_pos_precision(standard_seg_list, my_seg_list):
    ...
    my_random_seg_list = get_rand_seg(my_seg_pos_list)
    my_seg_pos_list = my_random_seg_list
