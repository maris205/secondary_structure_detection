...
def print_seg_node(word_list):
    pos_value_list = []
    pos_value_list.append(1) #begin node     
    for item in word_list:
        if len(item) == 1:
            pos_value_list.append(1)
        else:
            zeros = [0 for i in range(len(item)-1)]
            pos_value_list.extend(zeros)
            pos_value_list.append(1)
    
    #print
    for item in pos_value_list:
        print item
...
if __name__=="__main__":
    ...
    for i in range(0,len(line_list),3):
        ...
        try:
            ...
            my_seg_list = my_seg.mi_seg(seq_letter_list).split(" ")
        ...
        print_seg_node(standard_seg_list)
        #准确率
